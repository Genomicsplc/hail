...
class HailContext(HistoryMixin):
    ...
    .. warning::
        ...
        need to reconfigure settings, restart the Python session or use the :meth:`.HailContext.stop` method.
    ...
    :param default_reference: Default reference genome to use. Can be set to "GRCh37" for
                             :meth:`.GenomeReference.GRCh37`, "GRCh38" for :meth:`.GenomeReference.GRCh38`,
                             or the path to a JSON file for constructing a reference genome as described in
                             :meth:`.GenomeReference.read`.
    ...
    def import_bgen(self, path, tolerance=0.2, sample_file=None, min_partitions=None, reference_genome=None, contig_recoding=None):
        ...
        .. warning::
            ...
            A BGEN file must have a ``.idx`` file which can be generated by :meth:`~hail.HailContext.index_bgen`
        ...
        **Genotype probability (``gp``) representation**:
        ...
        The following modifications are made to genotype probabilities in BGEN v1.1 files:
          ...
          - Since genotype probabilities are understood to define a probability distribution, :meth:`~hail.HailContext.import_bgen` automatically sets to missing those genotypes for which the sum of the probabilities is a distance greater than the ``tolerance`` parameter from 1.0.  The default tolerance is 0.2, so a genotype with sum .79 or 1.21 is filtered out, whereas a genotype with sum .8 or 1.2 remains.

          - :meth:`~hail.HailContext.import_bgen` normalizes all probabilities to sum to 1.0. Therefore, an input distribution of (0.98, 0.0, 0.0) will be stored as (1.0, 0.0, 0.0) in Hail.

        **Annotations**

        :meth:`~hail.HailContext.import_bgen` adds the following variant annotations:
    ...
    def import_gen(self, path, sample_file=None, tolerance=0.2, min_partitions=None,
        ...
        **Notes**
        ...
        Also, see section in :meth:`~hail.HailContext.import_bgen` linked :ref:`here <gpfilters>` for information about Hail's genotype probability representation.

        **Annotations**

        :meth:`~hail.HailContext.import_gen` adds the following variant annotations:
    ...
    def import_table(self, paths, key=[], min_partitions=None, impute=False, no_header=False,
        ...
        **Notes**
        ...
        :type types: dict with str keys and :class:`.Type` values
    ...
    def import_vcf(self, path, force=False, force_bgz=False, header_file=None, min_partitions=None,
        ...
        **Notes**
        ...
        :meth:`~hail.HailContext.import_vcf` takes a list of VCF files to load. All files must have the same header and the same set of samples in the same order
        ...
        .. warning::
            ...
            - Not all :class:`.VariantDataset` methods will work with a generic genotype schema.

        :meth:`~hail.HailContext.import_vcf` does not perform deduplication - if the provided VCF(s) contain multiple records with the same chrom, pos, ref, alt, all
        ...
        .. note::
            ...
            filtering to PASS variants can be done with :meth:`.VariantDataset.filter_variants_expr`
            as follows:
    ...
    def index_bgen(self, path):
        """Index .bgen files. :meth:`.HailContext.import_bgen` cannot run without these indices.
    ...
    def balding_nichols_model(self, populations, samples, variants, num_partitions=None,
        ...
        **Annotations**
        ...
        :meth:`~hail.HailContext.balding_nichols_model` adds the following global, sample, and variant annotations:
