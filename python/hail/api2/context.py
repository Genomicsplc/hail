...
import hail
from hail.expr.types import Type
from hail.history import *
from hail.genetics import GenomeReference
from hail.stats.distributions import UniformDist, TruncatedBetaDist, BetaDist
...
class HailContext(HistoryMixin):
    ...
    def grep(self, regex, path, max_count=100):
        self._hc1.grep(regex, jindexed_seq_args(path), max_count)
    ...
    def import_table(self, paths, key=[], min_partitions=None, impute=False, no_header=False,
        ...
        return self._hc1.import_table(paths, key, min_partitions, impute, no_header, comment,
    ...
    def balding_nichols_model(self, populations, samples, variants, num_partitions=None,
        ...
        return self._hc1.balding_nichols_model(populations, samples, variants, num_partitions,
                                               pop_dist, fst, af_dist, seed, reference_genome).to_hail2()

    @handle_py4j
    @typecheck_method(expr=strlike)
    def eval_expr_typed(self, expr):
        self._hc1.eval_expr_typed(expr)

    @handle_py4j
    @typecheck_method(expr=strlike)
    def eval_expr(self, expr):
        self._hc1.eval_expr(expr)
